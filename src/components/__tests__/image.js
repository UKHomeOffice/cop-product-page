...
describe("image", () => {
  ...
  it("renders correctly when not on mobile", () => {
    const tree = renderer.create(<Image noMobile={true} />).toJSON();
    expect(tree).toMatchSnapshot();
  });

  it("Returns the correct source and alt with its correct values given", () => {
    const tree = renderer.create(<Image alt={"description"} source={link} />);
  ...
  it("Returns the correct class properties", () => {
    const tree = renderer.create(
      <Image noImage={true} alt={"description"} source={link} />
    );
    const testInstance = tree.root;
