    ...
    const tree = renderer
      .create(
        <StatContent
          ...
          stats={[
            { stat: "6,6525", description: "Border Force users" },
            {
              stat: "7,55500+",
  ...
  it("displays the service name", () => {
    const tree = renderer.create(
      <StatContent
        title={"Used and trusted by Border Force teams"}
        stats={[
          { stat: "6,652", description: "Border Force users1" },
          {
            stat: "7,500+",
            description: "forms and reports submitted per month2",
          },
    ...
    const getDescriptionProp = (key) => {
      return testInstance.findByProps({ id: key })._fiber.memoizedProps
        .children;
    };
    expect(getDescriptionProp("valueText0")).toEqual("Border Force users1");
    expect(getDescriptionProp("valueText1")).toEqual(
      "forms and reports submitted per month2"
    );
