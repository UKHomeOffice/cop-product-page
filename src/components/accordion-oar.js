    ...
    <div style={{ marginBottom: '2em' }} className="accordion">
      {props.content.map((value, index) => {
        return <div key={index} className="option">
          ...
          <label style={{ fontSize: '16px', marginBottom: '0px', backgroundColor: 'unset', color: '#0036a8', cursor: 'pointer', paddingBottom: '1em', paddingLeft: '0px' }} className="title govuk-heading-s" htmlFor={`${value.id}${value.header}${index}`}>{value.header}
          </label>
